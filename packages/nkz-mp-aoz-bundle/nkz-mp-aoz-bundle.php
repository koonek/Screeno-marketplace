...
define( 'NKZMP_AOZ_BUNDLE_VERSION', '0.71.0' );
...
register_activation_hook(
	...
	static function (): void {
		...
		// Pročistit cache po aktivaci/aktualizaci – LiteSpeed (a spol.) jinak
		// servíruje staré CSS/JS a změny vzhledu se „neprojeví".
		if ( class_exists( \NKZMP\Dashboard\CacheFlush::class ) ) {
			\NKZMP\Dashboard\CacheFlush::purge();
		}
...
register_deactivation_hook(
	...
	static function (): void {
		...
		if ( class_exists( \NKVSVS\Failed_Transfers::class ) ) {
			\NKVSVS\Failed_Transfers::unschedule();
		}
		if ( class_exists( \NKZMP\Packeta\Settings::class ) ) {
			\NKZMP\Packeta\Settings::unschedule();
		}
		if ( class_exists( \NKZMP\Packeta\StatusSync::class ) ) {
			\NKZMP\Packeta\StatusSync::unschedule();
		}
		flush_rewrite_rules();
	}
);
